...
func TestAdd(t *testing.T) {
	t.Run("EqWithInts", func(t *testing.T) {
		for _, tc := range tcInt {
			Eq(t, Add(tc.A, tc.B), tc.R)
		}
	})
	t.Run("EqWithStrs", func(t *testing.T) {
		for _, tc := range tcStr {
			Eq(t, Add(tc.A, tc.B), tc.R)
		}
	})
	t.Run("EqWithUsers", func(t *testing.T) {
		for _, tc := range tcUsers {
			Eq(t, tc.A == tc.B, tc.R)
		}
	})
	t.Run("InWithUsers", func(t *testing.T) {
		Eq(t, In(uu, uu[2]), true)
		Eq(t, In(uu, User{7, "User7"}), false)
	})
}
